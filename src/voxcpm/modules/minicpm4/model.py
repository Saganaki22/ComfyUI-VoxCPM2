...
class MiniCPMModel(nn.Module):
    ...
    def _get_position_emb(self, position_ids, seq_len=None):
        """Get position embeddings. Returns identity (cos=1, sin=0) if no_rope."""
        if self.no_rope:
            sl = seq_len if seq_len is not None else position_ids.shape[-1]
            device = position_ids.device if isinstance(position_ids, torch.Tensor) else position_ids
            dtype = next(self.parameters()).dtype if list(self.parameters()) else torch.float32
            cos = torch.ones(sl, self.head_dim, device=device, dtype=dtype)
            sin = torch.zeros(sl, self.head_dim, device=device, dtype=dtype)
            return cos, sin
    ...
    def forward(
        ...
        position_emb = self._get_position_emb(position_ids, seq_len=inputs_embeds.size(1))
    ...
    def forward_step(
        ...
        position_emb = self._get_position_emb(position_id, seq_len=1)
        hidden_states = inputs_embeds

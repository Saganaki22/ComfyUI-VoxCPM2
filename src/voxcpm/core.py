...
class VoxCPM:
    ...
    def _generate(self,
        ...
        # Both prompt audio and prompt text must be provided together (or neither)
        # The open-source VoxCPM2 model does not support audio-only cloning without a transcript
        if has_prompt_audio and prompt_text is None:
            raise ValueError("prompt_text (transcript) is required when providing reference audio. The open-source VoxCPM2 model needs both audio and its transcript for voice cloning.")

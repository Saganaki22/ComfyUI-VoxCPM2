...
class VoxCPMModel(nn.Module):
    def __init__(
        ...
        if config.residual_lm_no_rope:
            residual_lm_config.no_rope = True
        ...
        self.encoder_sample_rate = audio_vae.sample_rate
    ...
    def _generate(
        ...
        # VoxCPM2 controllable cloning: reference audio without transcript
        ...
        if not has_prompt:
            ...
            text = (prompt_text or "") + target_text
            ...
            enc_sr = self.encoder_sample_rate
            if sr != enc_sr:
                audio = torchaudio.functional.resample(audio, sr, enc_sr)
            ...
            # (B, D, T)
            # Encode on device
            audio_feat = self.audio_vae.encode(audio.to(device), enc_sr).cpu()
